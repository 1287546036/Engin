...
#define DM_MOTOR2_ID             0x02
#define DM_MST_ID2               0x12
#define DM_MOTOR3_ID             0x03
#define DM_MST_ID3               0x13
#define DM_MOTOR4_ID             0x04
#define DM_MST_ID4               0x14
...
extern DM_Motor_t Motor2;
extern DM_Motor_t Motor3;
extern DM_Motor_t Motor4;

void DM_Motor_Enable(CAN_HandleTypeDef* hcan, DM_Motor_t *motor);// 使能电机模式,包含enable函数
void DM_enable_motor_mode(CAN_HandleTypeDef* hcan, uint16_t motor_id, uint16_t mode_id);// 使能并发送模式
void DM_disable_motor_mode(CAN_HandleTypeDef* hcan, uint16_t motor_id, uint16_t mode_id);// 进入失能模式

void DM_Motor_Ctrl_Send(CAN_HandleTypeDef* hcan, DM_Motor_t *motor);// 根据模式发送目标值,包含以下函数
void DM_MIT_Ctrl(CAN_HandleTypeDef *hcan, DM_Motor_t *motor, float pos, float vel, float KP, float KD, float torq);// 延时整整200ms
...
void read_motor_data(uint16_t id, uint8_t rid);// 发送读取寄存器命令
void read_motor_ctrl_fbdata(uint16_t id);// 发送读取电机反馈数据的命令
...
void save_pos_zero(CAN_HandleTypeDef* hcan, uint16_t motor_id, uint16_t mode_id);// 保存位置零点函数
