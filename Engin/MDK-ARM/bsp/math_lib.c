...
{
    ramp_source_type->frame_period = frame_period;     //SHOOT_CONTROL_TIME * 0.001f
    ramp_source_type->max_value = max;                 //Fric_DOWN 1300
    ramp_source_type->min_value = min;                 //Fric_OFF 1000
    ramp_source_type->input = 0.0f;
    ramp_source_type->out = 0.0f;
}

/**
  * @brief          斜坡函数（版本一）计算，根据输入的值进行叠加， 输入单位为 /s 即一秒后增加输入的值
  * @author         RM
  * @param[in]      斜波函数结构体
  * @param[in]      输入值
  * @param[in]      滤波参数
...
/**
  * @brief  斜坡函数（版本二）,使目标输出值缓慢等于期望值
  *         就是说他可以在一定范围内改变达到期望值的速度
